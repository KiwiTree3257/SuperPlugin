...
import plugin.superplugin.customentity.WindStorm;
...
public class SuperJihwanFunction {
    private static String supername = "superjihwan";
    ...
    public static void SuperTransformation(Player player) {
        ...
        if (playerData.has(CustomKeys.Player_Super)) {
            ...
            if (36 - playerItemCount >= SuperJihwanItem.addItems.length) {
    ...
    public static void Transformation(Player player) {
        Inventory playerInventory = player.getInventory();
        playerInventory.addItem(SuperJihwanItem.addItems);
        player.getAttribute(Attribute.GENERIC_MAX_HEALTH).setBaseValue(40);

        JetGauge.addPlayer(player);
    ...
    public static void Untransformed(Player player) {
        ...
        for (int i = 0; i < SuperJihwanItem.removeItems.length; i++) {
            int[] indexes = Function.FindItemPDIndexesAtPlayerInventory(player, SuperJihwanItem.removeItems[i], PersistentDataType.INTEGER);
        ...
        if (player.getInventory().getItemInOffHand().getItemMeta() != null) {
            for (int i = 0; i < SuperJihwanItem.removeItems.length; i++) {
                if (Function.CompareItemPersistentData(player.getInventory().getItemInOffHand(), SuperJihwanItem.removeItems[i], PersistentDataType.INTEGER)) {
                    player.getInventory().setItemInOffHand(null);
                }
            }
        }

        JetGauge.removePlayer(player);
    ...
    public static void JihwanSkill_1(Player player, int delay) {
        PersistentDataContainer playerData = player.getPersistentDataContainer();

        if (Objects.equals(playerData.get(CustomKeys.Player_Super, PersistentDataType.STRING), supername)) {
            if (!player.getLocation().clone().add(0, -0.1, 0).getBlock().isCollidable()) {
                player.setVelocity(player.getVelocity().add(player.getLocation().getDirection().clone().normalize().multiply(-1)));
            }
            new WindStorm(player);
            CoolTimeManager.SetCoolTime(player, supername, 1, delay);
        }
    }
    public static void JihwanSkill_2(Player player, int delay) {
        PersistentDataContainer playerData = player.getPersistentDataContainer();
    ...
    public static void JihwanSkill_3(Player player, int delay) {
        PersistentDataContainer playerData = player.getPersistentDataContainer();
    ...
    public static void JihwanUltimate(Player player, int delay) {
        PersistentDataContainer playerData = player.getPersistentDataContainer();
